...
def encode_position(version: str, *, position: torch.Tensor, max_deg: int = 15):
    if version == "v1":
        ...
        return posenc_nerf(position, min_deg=0, max_deg=max_deg)
    else:
        raise ValueError(version)


def position_encoding_channels(version: Optional[str] = None, max_deg: int = 15) -> int:
    if version is None:
        return 1
    return encode_position(version, position=torch.zeros(1, 1), max_deg=max_deg).shape[-1]


class PosEmbLinear(nn.Linear):
    def __init__(
        self, posemb_version: Optional[str], in_features: int, out_features: int,
        max_deg: int = 15, **kwargs
    ):
        super().__init__(
            in_features * position_encoding_channels(posemb_version, max_deg=max_deg),
            out_features,
            **kwargs,
        )
    ...
    def forward(self, x: torch.Tensor):
        if self.posemb_version is not None:
            x = encode_position(self.posemb_version, position=x, max_deg=self.max_deg)
